...
public class PressTheKeypad {
	...
	public static void main(String[] args) {
		 ...
		 * 
		 * 이 전화 키패드에서 왼손과 오른손의 엄지손가락만을 이용해서 숫자만을 입력하려고 합니다. 맨 처음 왼손 엄지손가락은 * 키패드에 오른손
		 * 엄지손가락은 # 키패드 위치에서 시작하며, 엄지손가락을 사용하는 규칙은 다음과 같습니다.
		 * 
				...
				for(int k = 0; k<keyPad[0].length; k++) {
					if(numbers[i]==keyPad[j][k]) {
						if(numbers[i]==1||numbers[i]==4||numbers[i]==7) {
							...
							int ltempx = j-lMovePointX;
							int ltempy = k-lMovePointY;
							int rtempx = j-rMovePointX;
							int rtempy = k-rMovePointY;
							if(ltempx<0) {
								ltempx *= -1;
							}
							if(ltempy<0) {
							...
							if(ltempx+ltempy<rtempx+rtempy) {
								answer += "L";
								lMovePointX = j;
								lMovePointY = k;
								key = true;
								break;
							}else if(ltempx+ltempy>rtempx+rtempy) {
								answer += "R";
								rMovePointX = j;
								rMovePointY = k;
								key = true;
